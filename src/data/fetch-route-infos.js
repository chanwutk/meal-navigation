...
export const CURRENT_LOCATION = [37.87607, -122.258502];
export const stores= {
  'Whole Food': [
    {
      brand: 'Whole Food',
...
(async function () {
  await fetch('https://api.openrouteservice.org/v2/directions/driving-car').catch(d => console.log(d))
  console.log('[')
  for (const route of possibleRoutes) {
    ...
    for (const [from, to] of pair(path)) {
      const url = `https://api.openrouteservice.org/v2/directions/driving-car?api_key=${process.env.OPEN_ROUTE_SERVICE_KEY}&start=${from.slice()
        .reverse()
        .join(',')}&end=${to.slice().reverse().join(',')}`;
        // console.log(url)
      r.push(
        await fetch(url)
